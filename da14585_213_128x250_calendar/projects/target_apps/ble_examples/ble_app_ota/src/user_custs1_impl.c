...
{
// 定义图片绘制的坐标和尺寸
const UWORD img_x = 190; // 图片绘制的起始X坐标
...
Paint_NewImage(epd_buffer, EPD_2IN13_V2_WIDTH, EPD_2IN13_V2_HEIGHT, 270, WHITE);
            ...
            EPD_DrawUTF8(40, 108, 0, (const char *)buf2, EPD_ASCII_7X12, 0, BLACK, WHITE);
             // 获取并显示MAC地址后六位     
}
...
void do_display_update_with_analog_clock(void)
{
    ...
    EPD_DrawUTF8(250 - 23, 1 + 2, 0, buf, EPD_ASCII_7X12, 0, WHITE, BLACK);
    
    // 蓝牙连接状态指示
    if (isconnected == 1)
    {
        EPD_DrawUTF8(250 - 10, 108, 0, "B", EPD_ASCII_7X12, 0, WHITE, BLACK);
    }
